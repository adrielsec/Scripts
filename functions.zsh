...
jiraScan() {
  ...
  cat ALLHTTP | nuclei -t $HOME/custom_nuclei_templates/ssrf-jira-ell.yaml -H $UserAgent -o jiraNuclei
...
secretfinder() {
  ...
  regexs=$(curl -s 'https://gist.githubusercontent.com/adrielsec/1070fae0021b08d5e5650743ea402b4b/raw/589e669dec183009f01eca2c2ef1401b5f77af2b/regexJS' | tr '\n' '|')
...
fleetScan() {
  ...
  axiom-scan 'ell*' --rate=10000 -p443 --banners -iL=clean.subdomains -o=masscanIC.txt

  echo 'Clean file ...'

  cat masscanIC.txt | awk '{print $2}' | grep -v Masscan | grep -v Ports | sort -u | tee $liveHosts
  axiom-scan "ell*" -iL=$liveHosts -p443 -sV -sC -T4 -m=nmapx -o=output
